<?php
	...
	$meta_desc = '';
	$meta_keywords = '';
	$process_link = 'add_new_blog';
	if(isset($blog)){
		$title = $blog->title;
		$id_title = $blog->id_title;
		$date = date('d-m-Y',strtotime($blog->study_date));
		$blog_id = $blog->id;
				...
				<div class="article-input">
                    ...
                    <?php if($image) { ?>
                         <div class="file-preview" id="image-preview">
                                <span><?=$image;?></span> <i class="fa fa-search cursor" onmouseover="case_std.toggle_gallery_tooltip('tool_img_<?=$blog_id;?>','show');" onmouseout="case_std.toggle_gallery_tooltip('tool_img_<?=$blog_id;?>','hide');"></i> 
...
<script>
	$(function(){
	 ...
	 // help.custom_checkbox('.checker');
	  ...
	  //.make_permalink('#title','#id_title'); 
	  
	  $('#title').keyup(function(){
		    var permalink = $('#id_title').val();
		 	//help.permalink_exists('<?=base_url();?>blog/ajax/permalink_exists',permalink,'#id_title'); 
	  });
...
var case_std = {
	...
	delete_blog_doc:function(){
		var blog_id = $('#blog_id').val();
		if(blog_id){
			$.ajax({
				url: '<?=base_url();?>blog/ajax/delete_blog_doc',
				type: 'POST',
				data: {blog_id:blog_id},
				success: function(html) {
					if(html != 'failed'){
...
<div id="delete_category_model" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <button class="btn btn-info" onclick="case_std.delete_blog_category();"><i class="fa fa-trash-o"></i> Delete</button>
            </div>
        </div>
    </div>
</div>
...
<div id="delete_blog_image_model" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
            <button class="btn" data-dismiss="modal" aria-hidden="true">Cancel</button>
            <button class="btn btn-info" onclick="case_std.delete_blog_image();"><i class="fa fa-trash-o"></i> Delete</button>
            </div>
        </div>
    </div>
</div>
...
<div id="delete_blog_doc_model" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
            <button class="btn" data-dismiss="modal" aria-hidden="true">Cancel</button>
            <button class="btn btn-info" onclick="case_std.delete_blog_doc();"><i class="fa fa-trash-o"></i> Delete</button>
            </div>
        </div>
    </div>
</div>
