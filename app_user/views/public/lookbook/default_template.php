...
$(function(){
	...
	$(document).on('click','.like-this-staff',function(){
		var $this = $(this);
		var client_user_id = $this.attr('data-client-user-id');
		var staff_user_id = $this.attr('data-staff-user-id');
		like_this_staff(client_user_id,staff_user_id);
	});
});

function like_this_staff(client_user_id,staff_user_id){
	// 'btn-like-';
	var btn_obj = $('#btn-like-'+staff_user_id);
	$.ajax({
		type: "POST",
		url: "<?=base_url();?>plb/update_like_status",
		data: {client_user_id: client_user_id,staff_user_id:staff_user_id},
		dataType: "JSON",
		success: function(data) {
			if(data.liked == 'yes'){
				btn_obj.addClass('btn-core').html('<i class="fa fa-thumbs-o-down"></i> Un-Like This Staff');	
			}else{
				btn_obj.removeClass('btn-core').html('<i class="fa fa-thumbs-o-up"></i> Like This Staff');
			}
		}
	})
}
